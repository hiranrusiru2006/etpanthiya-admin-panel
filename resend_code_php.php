<?php
// Server-side resend verification code
session_start();
...
// If no phone number in POST, try to get from session
if (empty($phoneNumber) && isset($_SESSION['phone_number'])) {
    $phoneNumber = $_SESSION['phone_number'];
}

// Validate phone number
if (empty($phoneNumber)) {
    header('Location: index.php?error=Phone number not found. Please try again.');
    exit;
}
...
if ($response === false) {
    // An error occurred
    header("Location: index.php?verify=true&phone=$phoneNumber&error=Failed to connect to SMS gateway. Please try again.");
    exit;
}
...
if (strpos($response, 'OK:') === 0) {
    ...
    header("Location: index.php?verify=true&phone=$phoneNumber&success=Verification code resent successfully.");
} else {
    // Error - extract error message
    $errorMsg = strpos($response, ':') !== false ? explode(':', $response)[1] : 'Unknown error';
    header("Location: index.php?verify=true&phone=$phoneNumber&error=" . urlencode("SMS sending failed: " . trim($errorMsg)));
}
exit;
?>